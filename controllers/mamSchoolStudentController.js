...
exports.createImage = async (req, res) => {
  try {
    ...
    if (!attendanceRecord) {
      return res
        .status(400)
        .json({ error: "ID does not exist in mam_attendance" });
    }

    const { API_User_ID, Image_storage_path, Image_filename } =
      attendanceRecord;
    ...
    const { School_ID, Class_ID, Location_ID, School_Name, Class_Name } =
      schoolData;
    ...
    newStudent.Ref_Image_filepath = path.join(
      "/Users/apple/Desktop/CDOT/CIAS_DATA/ENROLL_IMG",
      imageFilename
    );
    ...
    if (!sourcePath || !destPath) {
      return res
        .status(500)
        .json({ error: "Source or destination path is invalid" });
    }
